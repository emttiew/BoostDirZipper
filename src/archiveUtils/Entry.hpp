...
enum class EntryType : char { Directory = '\x00', File = '\x01' };
...
class Entry {
 public:
  ...
  bool isDirectory() const { return entryType == EntryType::Directory; }
  virtual ~Entry() {}

 protected:
  explicit Entry(EntryType pEntryType) : entryType(pEntryType) {}
  explicit Entry(size_t pDataSize) : dataSize(pDataSize) {}
  explicit Entry(EntryType pEntryType, size_t pDataSize)
      : entryType(pEntryType), dataSize(pDataSize) {}
  EntryType entryType;
  size_t dataSize = 0;
};
}  // namespace archive_utils